    ...
    <div className="bg-white p-5 rounded-lg shadow-md ">
      ...
      <div className="flex flex-col gap-3 ">
        {services.map((service, index) => (
          <div key={index} className="flex justify-between gap-1 rounded-lg relative border border-solid border-gray-200">
            ...
            <div className="flex flex-col w-full border rounded-md">
              <img src={service.image} alt={service.title} className="rounded-md" />
              <p className="p-2 font-bold text-left">{service.title}</p>
            </div>
          </div>
        ))}
      </div>
