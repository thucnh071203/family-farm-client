...
export default function CreateGroupForm() {
    ...
    const validate = () => {
        ...
        if (!privacyType || !["public", "private"].includes(privacyType.toLowerCase())) {
    ...
    const handleCreateGroup = async (e) => {
        ...
        try {
            ...
            for (const member of selectedMembers) {
                try {
                    await instance.post(`/api/group-member/create/${groupId}/${member.accId}`, null, {
                        headers: {
                            Authorization: `Bearer ${token}`
                        }
                    });
    ...
    const handleAddMember = async (friend) => {
        try {
            const response = await instance.get(`/api/account/profile-another/${friend.accId}`, {
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                },
            });
    ...
    const handleRemoveMember = (accId) => {
        setSelectedMembers(selectedMembers.filter((member) => member.accId !== accId));
    };

    return (
        <div className="group-page-right w-full h-full flex flex-col pt-12 lg:mt-[120px] md:ml-[30%] mt-[63px] pr-4 pl-[8%] lg:pl-0">
            <form onSubmit={handleCreateGroup} className="create-group-container w-full max-w-[832px] h-screen">
                ...
                <div className="type-privacy-container flex flex-row gap-6 mt-[54px]">
                    ...
                    <div className="select-privacy h-10">
                        <select className="w-[190px] p-2 bg-[#3DB3FB] bg-opacity-25 text-[#3DB3FB] focus:outline-none focus:ring-0 focus:border-none cursor-pointer font-roboto"
                            name=""
                            id=""
                            value={privacyType}
                            ...
                            >
                                <div className="member-info flex flex-row items-center gap-2">
                                    <div className="member-avatar rounded-full w-[32px] h-[32px] overflow-hidden">
                                        <img src={member.avatar || '/default-avatar.png'} alt={member.fullName} />
                                    </div>
                ...
                <button className="create-button lg:w-[220px] mt-14 p-[10px] flex items-center justify-center gap-[10px] bg-[#3db3fb] rounded-sm hover:bg-[#50ace6] cursor-pointer"
                    type="submit"
                >
