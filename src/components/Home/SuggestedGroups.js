                ...
                {groups.map((group, index) => (
                    <div key={index} className="relative rounded-lg border border-solid border-gray-200">
                        <div className="w-full h-24 rounded-md bg-no-repeat bg-center bg-cover"
                            style={{ backgroundImage: `url(${group.group.groupBackground})` }}></div>
                        <p className="absolute text-sm text-gray-500 right-0 top-16 p-1 px-3 text-white font-bold bg-blue-400 rounded-l-full">Members: {group.numberInGroup}</p>
                        <div className="absolute top-16 flex items-end gap-3 m-4">
                            <img src={group.group.groupAvatar}
