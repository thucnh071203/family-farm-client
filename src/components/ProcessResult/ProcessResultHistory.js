...
const ProcessResultHistory = () => {
    ...
    // Sắp xếp theo createdAt (mới nhất trước) và lấy ngày mới nhất
    const sortedResults = stepResults.sort(
        (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
    );
    const latestDate = sortedResults[0]?.createdAt;

    // Gộp nội dung HTML và ảnh
    const combinedContent = sortedResults
        .map((result) => result.stepResultComment)
        .join("");
    const combinedImages = sortedResults
        .flatMap((result) => (result.stepResultImage ? JSON.parse(result.stepResultImage) : []));
                        ...
                        {combinedImages.length > 0 && (
                            <>
                                <h3 className="mt-4 text-lg font-semibold">
                                    Actual Image{combinedImages.length > 1 ? "s" : ""}
                                </h3>
                                <div className="grid grid-cols-3 gap-4 mt-2">
                                    {combinedImages.map((imgSrc, idx) => (
                                        <img
                                            key={idx}
                                            src={imgSrc}
                                            alt={`Result ${idx + 1}`}
